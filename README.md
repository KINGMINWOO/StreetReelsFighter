...
#### Jetson Nano에서 구동 시 어려움
- 다양한 호환성 문제 발생 -> 호환성 문제 해결하고 모델 성능 테스트 -> 현저히 낮은 모델 성능(4 ~ 7 FPS)
- Docker이미지로 호환성 문제 극복, Tensor RT를 사용하여 성능 향상 기대
- 하지만 여전히 호환성 문제 발생 & Tensor RT를 사용해도 눈에 띄는 성능 향상 없음
- 입력 영상 해상도 낮춤, 배치 사이즈 조절 등의 여러 대응 방안 시도 예정

#### Raspberry PI + Hailo 8에서 구동 시 어려움
- 처음 사용하는 장비와 라이브러리로, 공식 매뉴얼과 정보를 최대한 참고하였으나 구조 파악에 난항을 겪음.
- YOLOv8n-pose.pt -> onnx -> har -> hef 변환 까지는 성공
- 하지만 형식 호환 이슈로 작동 실패
- 외부에서 raspi5 + hailo 8 성공한 hef에서 dependency 추출, 사용하여 해결
- GStreamer를 사용해 쾌적한 모델 구동 구현 성공

## **➕ 개선 사항**

#### 점수 계산 로직 강화
- 부위 별 가중치를 조절하여 챌린지 별 점수 계산 디테일 조정
- 난이도 조절 기능을 추가

#### 캐릭터 변환 기능 강화
- 모델 최적화를 통해 안정적 캐릭터 변환
- 2D를 넘어 3D 캐릭터 변환 기능 구현

#### 엣지 환경에서 GUI까지
- 엣지 디바이스에서 GUI까지 구현한 버전 구동

## **💡 발전 가능성**
- 랭킹 시스템을 도입하여 온라인 점수 대결 플랫폼으로 발전
- 실시간 온라인 배틀 시스템
- 지정된 챌린지가 아닌 사용자가 선택한 챌린지로 대결
- 모바일 어플로 출시 -> 스마트폰만 있으면 어디서나 플레이
